...
METIZY_HTML = (
    '<p class="sell">Метизы MIKA удобнее продавать <strong>по задаче</strong>, а не по длинному списку размеров. Спросите: дерево, универсальный монтаж, кровля или сборка узла на болтах.</p>'
    '<h3 class="subhead">1. Дерево и потайной крепёж</h3>'
    + ggrid(
        gcard(
            "Саморезы потай оксид, крупный шаг",
            "размеры 3,5×16 … 4,8×152 · фасовка 1 кг / 3 кг",
            "Чёрные оксидированные под дерево/монтаж: шляпка утапливается. Спросите длину и во что крутят. К ним сразу биту PH2/PZ2. Фасовка 1 кг — «добить», 3 кг — если объём большой.",
        )
    )
    + '<h3 class="subhead">2. Универсальный монтаж (цинк)</h3>'
    + ggrid(
        gcard(
            "Шурупы универсальные цинк белый / жёлтый",
            "от 3,0×12 до 6,0×160 · руб/кг",
            "Универсальный крепёж с цинком: мебель, каркасы, смешанные материалы. Белый и жёлтый цинк — варианты покрытия. Подбирайте диаметр×длину под материал и нагрузку.",
        )
    )
    + '<h3 class="subhead">3. Кровля и профнастил</h3>'
    + ggrid(
        gcard(
            "Саморезы кровельные по металлу и по дереву",
            "цинк и цвета RAL 8017 / 7024 / 6005 · разные длины",
            "Кровля и профнастил. Сначала спросите основание: металл или дерево — от этого тип самореза. Затем цвет RAL по образцу/коду покрытия. Не путайте «по металлу» и «по дереву».",
        )
    )
    + '<h3 class="subhead">4. Сборка узлов (болт + гайка + шайба)</h3>'
    + ggrid(
        gcard(
            "Болты DIN933 · гайки DIN934 · шайбы DIN9021",
            "М6–М16 · шайбы d6–d16 · руб/кг",
            "Когда собирают раму, крепление, узел «на резьбе». Сверьте размер резьбы один в один. Продавайте комплектом: болт + гайка + шайба увеличенная — так меньше возвратов «не хватило шайб».",
        )
    )
    + '<h3 class="subhead">5. Длинный резьбовой крепёж</h3>'
    + ggrid(
        gcard(
            "Шпильки резьбовые оцинкованные DIN975",
            "кл.пр. 8.8 · Ø 8–16 · длина 1000/2000 мм",
            "Длинный пруток под сборку и крепление на расстоянии. Спросите диаметр и длину. Не путайте класс прочности и диаметр с болтом «коротким».",
        )
    )
)
