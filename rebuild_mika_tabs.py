...
OILS_HTML = gstack(
    ...
    grow(
        "assets/products/masla_2.jpg",
        "Масло для поршневых компрессоров",
        "арт. 33425/1 · 1 л",
        "В картер компрессора по уровню. Спросите модель и допуск из инструкции. Не подменяйте моторным 2T/4T для садовой техники.",
    ),
    grow(
        "assets/products/masla_3.jpg",
        "Трансмиссионное ТАД-17",
        "арт. 33414/1 · 1 л",
        "Для редукторов и узлов, где указано ТАД-17. Сверьте назначение по технике. Не путайте с моторным маслом двигателя.",
    ),
)

SOLVENTS_HTML = gstack(
    grow(
        "assets/products/rastvoriteli_2.jpg",
        "Уайт-спирит MIKA",
    ...
    grow(
        "assets/web/mika/solv_acetone.jpg",
        "Ацетон MIKA",
    ...
    grow(
        ...
        "Готовит стены под покраску/отделку, если пылят или сильно впитывают. Разница только в объёме канистры — подбирайте под площадь. На полке берите канистру с надписью «грунтовка», не растворитель.",
    ),
)

WHEEL_HTML = gstack(
    grow(
        "assets/products/tachki_1.jpg",
...
REELS_HTML = ggrid(
    gcard(
        ...
        "посадка М10×1.25 LF · модели А002…А034, В010, В021, С010",
...
METIZY_HTML = ggrid(
    gcard(
        "Саморезы потай оксид, крупный шаг MIKA",
        "размеры 3,5×16 … 4,8×152 · фасовка 1 кг / 3 кг",
        "Чёрные оксидированные под дерево/монтаж с потайной шляпкой. Разница в диаметре×длине и фасовке. Спросите, во что крутят и какая длина нужна. К ним сразу биту PH/PZ.",
    ),
    gcard(
        "Шурупы универсальные цинк (белый / жёлтый) MIKA",
        "размеры от 3,0×12 до 6,0×160 · руб/кг",
        "Универсальный крепёж с покрытием цинк. Белый и жёлтый цинк — варианты покрытия; подбирайте размер под материал. Спросите длину/диаметр и объём (кг).",
    ),
    gcard(
        "Саморезы кровельные по металлу и по дереву MIKA",
        "цинк и цвета RAL 8017 / 7024 / 6005 · разные длины",
        "Кровля и профнастил: по металлу или по дереву, часто в цвет RAL. Спросите основание и цвет покрытия. Предложите нужную длину и сверку цвета по образцу/коду.",
    ),
    gcard(
        "Шайбы увеличенные DIN9021 · гайки DIN934 · болты DIN933 MIKA",
        "диаметры М6–М16 и шайбы d6–d16 · руб/кг",
        "Метизы общего назначения. Сверьте размер резьбы/диаметр один в один. Продавайте комплектом болт+гайка+шайба, если собирают узел.",
    ),
    gcard(
        "Шпильки резьбовые оцинкованные DIN975 MIKA",
        "кл.пр. 8.8 · Ø 8–16 · длина 1000/2000 мм",
        "Длинный резьбовой пруток под сборку и крепление. Спросите диаметр и длину. Не путайте класс прочности и диаметр.",
    ),
)
